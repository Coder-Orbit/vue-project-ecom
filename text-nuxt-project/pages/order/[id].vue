...
// const selectedBrand = ref(1); // Holds the selected brand ID
...
// const category = ref([]); // Holds the dropdown options
// const selectedCategory = ref(null); // Holds the selected vendor ID

//         onMounted(async () => {
//             try {
//                 const response = await $fetch(`${EndPoint}/admin/${MasterKey}/category?`, {
//                     method: 'GET',
//                     headers: headers.value,
//                 });
//                 // Map API data to the required format
//                 category.value = response.data.map(b => ({
//                     name: b.name, // Dropdown label
//                     id: b.id, // Dropdown value
//                 }));
...
//             const selected = category.value.find(b => b.id === id);
//             return selected ? selected.name : '';
// };
  ...
  <NuxtLayout :name="layout">
      <div class="w-full px-3 mt-1">
          <div
              ...
              class="min-h-screen w-full top-0 left-0 z-30 flex items-center fixed"
          >
          ...
          >
              <div class="flex w-full justify-between bg-gray-400 text-white">
                  <div class="font-semibold mt-1 ml-3"><OrderStatus :status-i-d="statusID"/></div>
              ...
              <div
                  class="h-[calc(100vh-10.4rem)] overflow-y-auto border-b px-3 pt-3"
              >
                  <table class="w-full table-auto">
                      <thead>
                          <tr class="w-full bg-gray-300 text-sm">
                              ...
                              <th class="p-1 text-left text-sm">
                                  Contact No
                              </th>
                      ...
                      <tbody>
                          ...
                          >
                              ...
                              <td class="p-1 text-left text-xs">
                                  {{ order.extend_props.contact.name }}
                              </td>
                              <td class="p-1 text-left text-xs">
                                  {{ order.extend_props.contact.mobile }}
                              </td>
                              ...
                              <td
                                  class="p-1 text-xs grid text-center justify-items-center"
                              >
                                  <div class="flex">
                                      <NuxtLink
                                          class="bg-cyan-400 p-1 text-white rounded"
                                      ...
                                      <NuxtLink
                                          class="p-1 text-white rounded bg-red-500 ml-1"
                                          :to="`/order/print/${order.id}`"
                                      >
                                          <Icon
              ...
              <div class="order_title text-sm flex justify-between h-full">
                  ...
                  <div class="flex -mt-1 h-dvh">
                      <a
                          v-for="(page, index) in coreofOrders?.links"
                          :key="index"
          ...
          >
              ...
              <div class="w-full grid grid-cols-2 gap-2 mt-2">
                  ...
                  <!-- <div>
                      ...
                      >
                          <template #value="slotProps">
                              <div v-if="slotProps.value" class="flex align-items-center">
                                  <div>{{ getCategoryName(slotProps.value) }}</div>
                              </div>
                              <span v-else>
                                  {{ slotProps.placeholder }}
                              </span>
              ...
              <div class="mt-2 text-sm text-gray-600">
                  <span v-if="totalOrders"
                      >Showing {{ totalOrders }} results</span
                  >
